...
class Ctx(conv.State):
    ...
    env_keys = ('_lang', '_session', '_translator', '_user')

    def __init__(self, req = None):
        if req is not None:
            self.req = req
            weotu_ui_env = req.environ.get('weotu-ui', {})
            for key in object.__getattribute__(self, 'env_keys'):
                value = weotu_ui_env.get(key)
    ...
    def blank_req(self, path, environ = None, base_url = None, headers = None, POST = None, **kw):
        ...
        weotu_ui_env = env.setdefault('weotu-ui', {})
        for key in self.env_keys:
            value = getattr(self, key)
            if value is not None:
                weotu_ui_env[key] = value
    ...
    def lang_del(self):
        del self._lang
        if self.req is not None and self.req.environ.get('weotu-ui') is not None \
                and '_lang' in self.req.environ['weotu-ui']:
            del self.req.environ['weotu-ui']['_lang']

    def lang_get(self):
        if self._lang is None:
            self._lang = ['en-US', 'en']
            if self.req is not None:
                self.req.environ.setdefault('weotu-ui', {})['_lang'] = self._lang
        return self._lang
    ...
    def lang_set(self, lang):
        ...
        if self._translator is not None:
            ...
            if self.req is not None and self.req.environ.get('weotu-ui') is not None \
                    and '_translator' in self.req.environ['weotu-ui']:
                del self.req.environ['weotu-ui']['_translator']
    ...
    def session_del(self):
        del self._session
        if self.req is not None and self.req.environ.get('weotu-ui') is not None \
                and '_session' in self.req.environ['weotu-ui']:
            del self.req.environ['weotu-ui']['_session']
    ...
    def session_set(self, session):
        ...
        if self.req is not None:
            self.req.environ.setdefault('weotu-ui', {})['_session'] = session
    ...
    def user_del(self):
        del self._user
        if self.req is not None and self.req.environ.get('weotu-ui') is not None \
                and '_user' in self.req.environ['weotu-ui']:
            del self.req.environ['weotu-ui']['_user']
    ...
    def user_set(self, user):
        self._user = user
        if self.req is not None:
            self.req.environ.setdefault('weotu-ui', {})['_user'] = user
